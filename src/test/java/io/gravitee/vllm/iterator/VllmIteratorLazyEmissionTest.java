...
class VllmIteratorLazyEmissionTest {
    ...
    private static VllmOutput streamingOutput(String requestId, String text, String delta) {
        return new VllmOutput(requestId, text, delta, false, null, null, List.of(), null);
    }

    private static VllmOutput finishedOutput(String requestId, String text, String delta,
                                             List<Integer> tokenIds,
                                             List<Map<Integer, LogprobEntry>> logprobs) {
        return new VllmOutput(requestId, text, delta, true, "stop", null, tokenIds, logprobs);
    }
    ...
    void tokenIds_shouldBeEmittedOnFinish() {
        var output = finishedOutput("req-1", "hello", "o", List.of(10, 20, 30), null);
    ...
    void tokenIds_onlyLastOutputHasTokens() {
        ...
        var step3 = finishedOutput("req-1", "hello", "o", List.of(10, 20, 30), null);

        var allOutputs = List.of(step1, step2, step3);

        // Only the finished output has token IDs
        var withTokens = allOutputs.stream()
    ...
    void logprobs_shouldBeEmittedOnFinishWhenRequested() {
        ...
        var output = finishedOutput("req-1", "hello", "o", List.of(10, 20, 30), logprobs);

        assertThat(output.finished()).isTrue();
        assertThat(output.logprobs()).hasSize(3);
        assertThat(output.logprobs().get(0).get(10).decodedToken()).isEqualTo("hel");
    ...
    void logprobs_onlyLastOutputHasLogprobs() {
        ...
        var step1 = streamingOutput("req-1", "hel", "hel");
        var step2 = streamingOutput("req-1", "hell", "l");
        var step3 = finishedOutput("req-1", "hello", "o", List.of(10, 20, 30), logprobs);

        var allOutputs = List.of(step1, step2, step3);

        var withLogprobs = allOutputs.stream()
    ...
    void tokenIds_countShouldMatchLogprobsPositions() {
        ...
        var output = finishedOutput("req-1", "hello", "lo", List.of(10, 20), logprobs);
