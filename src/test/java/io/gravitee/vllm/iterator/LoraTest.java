...
class LoraTest {
    ...
    private static final String LORA_REPO = "gauravprasadgp/Qwen3-0.6B_nlp_to_sql";
    ...
    void generate_with_lora_adapter_from_huggingface() {
        ...
        String prompt = chatTemplate.render(List.of(
                ChatMessage.system("You are a SQL expert. Convert natural language queries to SQL. /no_think"),
        ...
        try (var sp = new SamplingParams().temperature(0.0).maxTokens(128)) {
            ...
            System.out.println("\n=== LoRA Generate (SQL) ===");
            System.out.println("Prompt: Show all employees with salary greater than 50000");
    ...
    void parallel_lora_and_base_model_requests() {
        ...
        String loraPrompt = chatTemplate.render(List.of(
                ChatMessage.system("Convert to SQL. /no_think"),
                ChatMessage.user("Get all orders from last month")
        ), true);

        String basePrompt = chatTemplate.render(List.of(
                ChatMessage.user("What is 2 + 2? /no_think")
        ), true);
        ...
        try (var sp1 = new SamplingParams().temperature(0.0).maxTokens(64);
            ...
            iterator.addRequest(new VllmRequest("req-base-parallel", basePrompt, sp2));

            var loraOutput = new StringBuilder();
            var baseOutput = new StringBuilder();

            iterator.stream().forEach(o -> {
