...
class StateEvaluationTest {
    ...
    void uninitializedFsm_shouldReturnCurrentState() {
        ...
        assertThat(fsm.evaluate(GenerationState.ANSWER, "hello")).isEqualTo(GenerationState.ANSWER);
    }

    @Test
    void nullDelta_shouldReturnCurrentState() {
        initWithReasoningTags();
        assertThat(fsm.evaluate(GenerationState.ANSWER, null)).isEqualTo(GenerationState.ANSWER);
    }

    @Test
    void emptyDelta_shouldReturnCurrentState() {
        initWithReasoningTags();
        assertThat(fsm.evaluate(GenerationState.ANSWER, "")).isEqualTo(GenerationState.ANSWER);
    }
    ...
    void noTagsInText_shouldStayInAnswer() {
        ...
        var state = fsm.evaluate(GenerationState.ANSWER, "just normal text without tags");
        assertThat(state).isEqualTo(GenerationState.ANSWER);
    }
