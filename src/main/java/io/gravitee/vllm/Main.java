...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    String venvPath = params.getOrDefault(
      "venv",
      System.getProperty("user.dir") + "/.venv"
    );

    // Engine parameters — CLI overridable, with small-GPU-friendly defaults
    boolean enforceEager = Boolean.parseBoolean(
      params.getOrDefault("enforce_eager", "true")
    );
    int maxModelLen = Integer.parseInt(
      params.getOrDefault("max_model_len", "4096")
    );
    int maxNumSeqs = Integer.parseInt(
      params.getOrDefault("max_num_seqs", "16")
    );
    double gpuMemoryUtilization = Double.parseDouble(
      params.getOrDefault("gpu_memory_utilization", "0.85")
    );
    ...
    double repetitionPenalty = Double.parseDouble(
      params.getOrDefault("repetition_penalty", "1.0")
    );
    double frequencyPenalty = Double.parseDouble(
      params.getOrDefault("frequency_penalty", "0.0")
    );
    double presencePenalty = Double.parseDouble(
      params.getOrDefault("presence_penalty", "0.0")
    );
    ...
    System.out.printf(
      "vLLM4j%n  model   : %s%n  dtype   : %s%n  venv    : %s%n  backend : %s%n",
      model,
      dtype,
      venvPath,
      VllmBackend.detect()
    ...
    while (!input.trim().equals("bye")) {
      ...
      try (
        ...
        ConversationState conversationState = newConversationState(
          tagBoundsConfig
        );
  ...
  private static List<TagBounds> parseTagConfig(Map<String, String> params) {
    ...
    if (reasoningTags != null) {
      ...
      if (parts.length == 2 && !parts[0].isEmpty() && !parts[1].isEmpty()) {
        ...
        System.err.println(
          "[WARN] Invalid --reasoning_tags format. Expected \"<open>|<close>\""
        );
      }
    }
    ...
    if (toolTags != null) {
      ...
      if (parts.length == 2 && !parts[0].isEmpty() && !parts[1].isEmpty()) {
        ...
        System.err.println(
          "[WARN] Invalid --tool_tags format. Expected \"<open>|<close>\""
        );
      }
    }
   ...
   */
  private static ConversationState newConversationState(
    List<TagBounds> tagBoundsConfig
  ) {
    if (tagBoundsConfig.isEmpty()) {
      return null;
    }
