...
public final class SamplingParams implements AutoCloseable, Freeable {
    ...
    public SamplingParams stopTokenIds(List<Integer> tokenIds) {
        return stopTokenIds(tokenIds.stream().mapToInt(Integer::intValue).toArray());
    }
    ...
    public SamplingParams includeStopStrInOutput(boolean include) {
        checkNotBuilt();
        try (var gil = GIL.acquire()) {
            PythonTypes.putDictObj(arena, kwargs, "include_stop_str_in_output",
    ...
    public SamplingParams skipSpecialTokens(boolean skip) {
        checkNotBuilt();
        try (var gil = GIL.acquire()) {
            PythonTypes.putDictObj(arena, kwargs, "skip_special_tokens",
    ...
    public SamplingParams spacesBetweenSpecialTokens(boolean spaces) {
        checkNotBuilt();
        try (var gil = GIL.acquire()) {
            PythonTypes.putDictObj(arena, kwargs, "spaces_between_special_tokens",
    ...
    public SamplingParams truncatePromptTokens(int maxPromptTokens) {
        checkNotBuilt();
        try (var gil = GIL.acquire()) {
            PythonTypes.putDictInt(arena, kwargs, "truncate_prompt_tokens", maxPromptTokens);
        }
        return this;
    }
    ...
    public SamplingParams stop(String... sequences) {
        checkNotBuilt();
        try (var gil = GIL.acquire()) {
            MemorySegment pyStopList = CPython.PyList_New(0);
            for (String seq : sequences) {
                MemorySegment pyItem = PythonTypes.pyStr(arena, seq);
                CPython.PyList_Append(pyStopList, pyItem);
