...
public final class ChatTemplate {
  ...
  private String renderTemplate(
    ...
    try (var gil = GIL.acquire()) {
      ...
      MemorySegment envClass = PythonTypes.getAttr(
        arena,
        jinja2Module,
        "Environment"
      );
      MemorySegment emptyEnvKwargs = CPythonBinding.PyDict_New();
      ...
      MemorySegment tmpl = PythonCall.callMethodObjArgs(
        env,
        fromStringName,
        pyTemplate
      );
      ...
      MemorySegment pyBool = CPythonBinding.PyBool_FromLong(
        addGenerationPrompt ? 1L : 0L
      );
      ...
      MemorySegment rendered = PythonCall.pyObjectCall(
        renderFn,
        emptyTuple,
        kwargs
      );
  ...
  private MemorySegment messagesToPyList(List<ChatMessage> messages) {
    ...
    for (ChatMessage msg : messages) {
      ...
      if (msg.hasContentParts()) {
        ...
        MemorySegment pyContentList = CPythonBinding.PyList_New(0);
        for (Map<String, Object> part : msg.contentParts()) {
          MemorySegment pyPart = mapToPyDict(part);
          CPythonBinding.PyList_Append(pyContentList, pyPart);
      ...
      if (msg.toolCallId() != null) {
        ...
        CPythonBinding.PyDict_SetItemString(
          pyDict,
          arena.allocateFrom("tool_call_id"),
          pyCallId
        );
      ...
      CPythonBinding.PyList_Append(pyList, pyDict);
  ...
  private MemorySegment toolsToPyList(List<Tool> tools) {
    ...
    for (Tool tool : tools) {
      MemorySegment pyTool = CPythonBinding.PyDict_New();
      ...
      // function dict
      MemorySegment pyFunc = CPythonBinding.PyDict_New();
      MemorySegment pyFuncName = PythonTypes.pyStr(
        arena,
        tool.function().name()
      );
      CPythonBinding.PyDict_SetItemString(
      ...
      MemorySegment pyFuncDesc = PythonTypes.pyStr(
        arena,
        tool.function().description()
      );
      CPythonBinding.PyDict_SetItemString(
        pyFunc,
        arena.allocateFrom("description"),
        pyFuncDesc
      );
      ...
      CPythonBinding.PyDict_SetItemString(
        pyTool,
        arena.allocateFrom("function"),
        pyFunc
      );
      PythonTypes.decref(pyFunc);

      CPythonBinding.PyList_Append(pyList, pyTool);
  ...
  private MemorySegment toolCallsToPyList(List<ToolCall> toolCalls) {
    ...
    for (ToolCall tc : toolCalls) {
      MemorySegment pyCall = CPythonBinding.PyDict_New();

      MemorySegment pyId = PythonTypes.pyStr(arena, tc.id());
      CPythonBinding.PyDict_SetItemString(
        pyCall,
        arena.allocateFrom("id"),
        pyId
      );
      ...
      PythonTypes.decref(pyName);
      MemorySegment pyArgs = PythonTypes.pyStr(
        arena,
        tc.function().arguments()
      );
      CPythonBinding.PyDict_SetItemString(
        pyFunc,
        arena.allocateFrom("arguments"),
        pyArgs
      );
      ...
      CPythonBinding.PyList_Append(pyList, pyCall);
